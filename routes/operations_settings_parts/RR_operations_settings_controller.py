...
def _update_settings_without_legacy_rent(shelter: str, row, form) -> None:
    now = utcnow_iso()
    ...
    pass_deadline_weekday = min(
        max(_merge_int("pass_deadline_weekday", form, row.get("pass_deadline_weekday"), 0), 0),
        6,
    )
    pass_deadline_hour = min(
        max(_merge_int("pass_deadline_hour", form, row.get("pass_deadline_hour"), 8), 0),
        23,
    )
    pass_deadline_minute = min(
        max(_merge_int("pass_deadline_minute", form, row.get("pass_deadline_minute"), 0), 0),
        59,
    )
    ...
    pass_work_required_hours = max(
        _merge_int("pass_work_required_hours", form, row.get("pass_work_required_hours"), 29),
        0,
    )
    ...
    db_execute(
        ...
        (
            ...
            inspection_default_item_status,
            inspection_item_labels,
            _bool_db(inspection_scoring_enabled, is_pg),
            inspection_lookback_months,
            _bool_db(inspection_include_current_open_month, is_pg),
            inspection_score_passed,
            _bool_db(inspection_needs_attention_enabled, is_pg),
            inspection_score_needs_attention,
            inspection_score_failed,
            inspection_passing_threshold,
            inspection_band_green_min,
...
def settings_section_page(section_key: str):
    ...
    if request.method == "POST":
        ...
        _update_settings_without_legacy_rent(shelter, row, form)
