...
def staff_login():
    ...
    if is_ip_banned(ip):
        log_action(
            ...
            details={"ip": ip, "username": safe_username},
        )
    ...
    if not is_active or not check_password_hash(pw_hash, password):
        ...
        log_action(
            ...
            details={
                "ip": ip,
                "reason": "bad_password_or_inactive",
                "username": safe_username,
            },
        )
        flash("Invalid login.", "error")
    ...
    if shelter not in allowed_shelters:
        log_action(
            ...
            details={
                "ip": ip,
                "reason": "invalid_shelter_for_user",
                "shelter": shelter,
                "username": safe_username,
            },
        )
    ...
    log_action(
        ...
        details={
            "ip": ip,
            "username": _safe_log_value(session["username"]),
        },
    )
...
def staff_logout():
    ...
    log_action(
        ...
        staff_id,
        "logout",
        details={"username": _safe_log_value(session.get("username"))},
    )
    session.clear()
...
def staff_profile():
    ...
    if request.method == "POST":
        ...
        log_action(
            ...
            staff_id,
            "profile_update",
            details={"source": "staff_profile", "updated_password": bool(password)},
        )
