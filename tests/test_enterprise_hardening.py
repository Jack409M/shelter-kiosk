...
def test_enterprise_hardening_preserves_existing_application_security_headers():
    app = _build_hardening_app(testing=True, debug=False)

    @app.after_request
    def _inject_custom_header(response):
    ...
    # register_app_hooks uses setdefault; an explicit, app-owned policy should not be overwritten.
    assert response.headers["Content-Security-Policy"] == "default-src 'self'"



def test_enterprise_hardening_does_not_set_hsts_in_debug_mode():
    app = _build_hardening_app(testing=False, debug=True)
    client = app.test_client()
...
def test_enterprise_hardening_propagates_existing_request_id_header():
    app = _build_hardening_app(testing=True, debug=False)
    client = app.test_client()

    response = client.get("/", headers={"X-Request-ID": "enterprise-correlation-id"})

    assert response.status_code == 200
    assert response.headers["X-Request-ID"] == "enterprise-correlation-id"
